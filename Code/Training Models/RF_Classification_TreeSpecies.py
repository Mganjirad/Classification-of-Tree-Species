...
def hyperopt(param_space, X_train, y_train, num_eval):
    ...
    def objective_function(params):
        calssifier = RandomForestClassifier(**params)
        score = cross_val_score(calssifier, Xtrain, ytrain, cv=5).mean()
    ...
    calssifier_best = RandomForestClassifier(n_estimators=int(best_param['n_estimators']),
    ...
    calssifier_best.fit(Xtrain, ytrain)
    ...
    return trials, calssifier_best,TimeElapsed,BestPmts

def C_FI(Model,Scheme):
    ...
    feature_importance_df.to_csv(f'FI_RF_{Scheme}.csv', index=False)
...
for i in TargetPmt:
    ...
    for j in Mode:
        ...
        param_hyperopt= {
            ...
            # A fraction of number of variables is used in each split
            'max_features': hp.uniform('max_features', 0.6, 1.0), 
            # The minimum number of samples required to be at a leaf node.
            #'min_samples_leaf': hp.uniform('min_samples_leaf', 0, 1),
            # The minimum number of samples required to split an internal node:
            #'min_samples_split': hp.uniform('min_samples_split', 0, 1),
            # If bootstrap is True, the number of samples to draw from training to train each base estimator.
            'max_samples': hp.uniform('max_samples', 0.6, 1.0),}
        results_hyperopt, calssifier_best,TimeElapsed,BestPmts = hyperopt(param_hyperopt, Xtrain, ytrain, 20)
        Scheme=i+'_'+j
        with open(f'RF_{Scheme}.pkl','wb') as f:
            pickle.dump(calssifier_best,f)
            print("Model Saved!")
        Scheme=i+'_'+j
        C_FI(calssifier_best,Scheme)
        ########################################################################################
        # Prediction (Training)
        ...
        #########################################################################################
    Df_Test.to_csv(f'RF_Test_{i}.csv')
    Df_Train.to_csv(f'RF_Train_{i}.csv')
    #########################################################################################
